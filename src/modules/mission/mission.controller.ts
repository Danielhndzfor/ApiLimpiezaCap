...
import * as missionRepository from './mission.repository';
...
export async function getMissionHandler(req: Request, res: Response, next: NextFunction) {
  try {
    const { rows } = await missionRepository.getMission();
...
export async function updateMissionHandler(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  try {
    const { body } = req.body ?? {};
    if (!body) throw new AppError('body es obligatorio', 400);
    const { status } = await missionRepository.updateMission(String(body), req.user!.idUser);
    handleStatus(status);
