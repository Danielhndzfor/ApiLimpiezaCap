...
export async function updateContactPageHandler(req: Request, res: Response, next: NextFunction) {
  try {
    const input = parseInput(req.body);
    const { status } = await contactPageRepository.updateContactPage(input);
    handleStatus(status);
