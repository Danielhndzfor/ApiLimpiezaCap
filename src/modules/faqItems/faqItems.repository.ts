...
async function callCrud(
  ...
  const [result] = await pool.query<RowDataPacket[][]>(
    ...
    [opc, idFaqItem, input?.question ?? null, input?.answer ?? null, idUser]
  );
...
export async function createFaqItem(input: FaqItemInput, idUser: number) {
  return callCrud(OPC.INSERT, null, input, idUser);
}

export async function updateFaqItem(idFaqItem: number, input: FaqItemInput, idUser: number) {
  return callCrud(OPC.UPDATE, idFaqItem, input, idUser);
}

export async function deleteFaqItem(idFaqItem: number) {
  return callCrud(OPC.DELETE, idFaqItem, null, null);
}
