...
export async function createSecurityMeasureHandler(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  try {
    const { status } = await securityMeasuresRepository.createSecurityMeasure(parseText(req.body), req.user!.idUser);
    handleStatus(status);
...
export async function updateSecurityMeasureHandler(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  try {
    const id = Number(req.params.id);
    const { status } = await securityMeasuresRepository.updateSecurityMeasure(id, parseText(req.body), req.user!.idUser);
    handleStatus(status);
