...
export async function createAdvantage(text: string) {
  return callCrud(OPC.INSERT, null, text);
}

export async function updateAdvantage(idAdvantage: number, text: string) {
  return callCrud(OPC.UPDATE, idAdvantage, text);
}

export async function deleteAdvantage(idAdvantage: number) {
  return callCrud(OPC.DELETE, idAdvantage, null);
}
